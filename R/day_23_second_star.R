...
mat <- matrix(unlist(str_split(x, "")), nrow = 7, byrow = T)

ss <- rep(c(1, 1, 10, 10, 100, 100, 1000, 1000), each = 2)
mat_empty <- mat
...
possible_moves_vec <- c(9, 16, 30, 44, 58, 72, 79) 
home_positions <- matrix(c(rep(24:27, times = 4), rep(38:41, times = 4), rep(52:55, times = 4), rep(66:69, times = 4)), ncol = 4, nrow = 16, byrow = T) #change

home_vec <- c(24:27, 38:41, 52:55, 66:69)

nr <- nrow(mat)
home_vals <- list(as.character(24:27), as.character(38:41), as.character(52:55), as.character(66:69))

possible_moves_vec <- as.character(possible_moves_vec)
...
reset <- function() {
  bb <- data.frame(am = rep(LETTERS[1:4], each = 4),
...
print_mat <- function(bb) {
  mat_empty[bb$pos] <- paste0(rep(LETTERS[1:4], each = 4), 1:16) #change
  print(mat_empty)
}
bb$pos
print_mat(bb)
...
best_score <- Inf
for(i in 1:30) {
  bb <- data.table::rbindlist(future_lapply(1:(nrow(bb)/(16)), function(rr) {
    bb_temp <- bb[ ((rr - 1) * 16 + 1) : (rr * 16 ), ]
    ff <- find_moves(bb_temp, 1:16)
    ...
    bind_rows(lapply(1:16, function(x) {
      if(length(ff[[x]]) == 0) {
        return(data.frame(am = character(0), pos = integer(0), scores = numeric(0), moves = numeric(0)))
      }
  ...
  bb <- bb %>% 
    mutate(game = rep(1:(n()/16),  each = 16)) %>% 
    group_by(game) %>% 
  ...
  if(i > 16) {
    tot_scores <- future_lapply(1:(nrow(bb)/(16)), function(rr) {
      bb_temp <- bb[ ((rr - 1) * 16 + 1) : (rr * 16 ), ]
      ff <- find_moves(bb_temp, 1:16)
      if(length(unlist(ff)) == 0) {
        if(length(intersect(bb_temp$pos, home_vec)) == 16) { #change this
